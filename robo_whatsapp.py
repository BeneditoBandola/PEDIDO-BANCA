...
def limpar_texto_pdf(texto):
    if not texto:
        return ""
    return re.sub(r'[^\w\s\-\(\)\.,/:;áéíóúãõâêîôûçÁÉÍÓÚÃÕÂÊÎÔÛÇ]', '', str(texto)).strip()

def interpretar_e_gerar_pedido(texto_wpp, nome_cliente="(35) WhatsApp"):
    # 1. Interpretação dos itens (mesmo motor inteligente que construímos)
    carrinho = {}
    ...
    if not carrinho:
        return "Nenhum produto identificado na mensagem."

    # 2. Numeração Sequencial e Histórico
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 4, "Mercado Municipal - Box 43 a 48 | Poços de Caldas - MG", 0, 1, "L")
            self.ln(6)
        ...
        def footer(self):
            ...
            self.cell(0, 10, f"Comprovante Automático - Página {self.page_no()}", 0, 0, "C")
    ...
    pdf.cell(190, 7, limpiar_texto_pdf(f" PEDIDO AUTOMATICO: {codigo_pedido}"), 1, 1, "C", fill=True)
    
    pdf.set_font("Arial", "B", 9)
    pdf.set_text_color(0, 0, 0)
    pdf.set_fill_color(245, 247, 246)
    pdf.cell(150, 6, limpiar_texto_pdf(f" CLIENTE / ORIGEM: {nome_cliente}"), 1, 0, "L", fill=True)
    ...
    for p, q in carrinho.items():
        pdf.set_fill_color(250, 250, 250) if fill_toggle else pdf.set_fill_color(255, 255, 255)
    ...
    # 4. Disparo do E-mail para Andréia e Benedito
    enviar_email_automatico(pdf_path, codigo_pedido, nome_cliente)
    return f"Pedido {codigo_pedido} gerado e enviado com sucesso!"
    ...
    try:
        ...
        for destinatario in destinatarios:
            ...
            with open(pdf_path, "rb") as f:
                ...
                parte.add_header('Content-Disposition', f'attachment; filename="pedido_whatsapp.pdf"')
                msg.attach(parte)
