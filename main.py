...
def enviar_email_automatico(pdf_path, codigo_pedido, cliente_nome):
    ...
    smtp_port = 587
    # Pega das variáveis de ambiente do servidor seguro
...
def receber_mensagem():
    dados = request.json
    # Extrai o texto da mensagem enviado pelo Make.com
    texto = dados.get("mensagem", "")
    remetente = dados.get("remetente", "Cliente WhatsApp")
    
    if not texto:
        return jsonify({"status": "erro", "detalhe": "Nenhuma mensagem encontrada"}), 400
